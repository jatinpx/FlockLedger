...
import { apiFetch, type EggProduction, type Paginated } from "../lib/api";
...
export function ProductionListScreen({ route }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await apiFetch<Paginated<EggProduction>>(
  ...
  return (
    <View style={styles.wrap}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? <Text style={styles.empty}>No production rows on this page.</Text> : null
        }
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>
              {item.date} · shed #{item.shed_id}
            </Text>
            <Text style={styles.cardBody}>
              {item.eggs_produced} eggs ({item.broken_eggs} broken) → {item.usable_eggs} usable ·{" "}
              {item.trays} trays
            </Text>
          </View>
        )}
